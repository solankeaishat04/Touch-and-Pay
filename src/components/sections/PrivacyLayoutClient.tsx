...
interface PrivacyLayoutClientProps {
  title: string;
  lastUpdated: string;
  sections: Section[];
}

export default function PrivacyLayoutClient({ title, lastUpdated, sections }: PrivacyLayoutClientProps) {
  ...
  useEffect(() => {
    ...
    const handleIntersection = (entries: IntersectionObserverEntry[]) => {
      ...
      if (visibleSections.length > 0) {
        // Track the component closest to the top viewport edge
    ...
    const observer = new IntersectionObserver(handleIntersection, {
      rootMargin: "-12% 0px -68% 0px", // Snaps cleanly right as the header line passes
      threshold: 0,
    });
      ...
      <div className="grid grid-cols-1 gap-12 lg:grid-cols-12 items-start relative">
        ...
        <aside className="lg:col-span-4 sticky top-28 hidden lg:block max-h-[calc(100vh-12rem)] overflow-y-auto pr-4 scrollbar-thin">
          <div className="flex items-center gap-2 mb-6 border-b border-neutral-100 pb-4">
            <ShieldCheck className="w-5 h-5 text-[#E87C36]" />
            <span className="text-xs font-black uppercase tracking-wider text-neutral-400">Privacy Control</span>
          </div>
