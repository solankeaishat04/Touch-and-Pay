...
interface DataProtectionClientProps {
  title: string;
  lastUpdated: string;
  sections: Section[];
}

export default function DataProtectionClient({ title, lastUpdated, sections }: DataProtectionClientProps) {
  const [activeSlug, setActiveSlug] = useState<string>(() => {
    return sections && sections.length > 0 ? sections[0].slug.current : "";
  });
      ...
      <div className="grid grid-cols-1 gap-12 lg:grid-cols-12 items-start relative">
        ...
        <aside className="lg:col-span-4 sticky top-28 hidden lg:block max-h-[calc(100vh-12rem)] overflow-y-auto pr-4 scrollbar-thin">
          <div className="flex items-center gap-2 mb-6 border-b border-neutral-100 pb-4">
            <Lock className="w-4 h-4 text-[#E87C36]" />
            <span className="text-xs font-black uppercase tracking-wider text-neutral-400">Security Policies</span>
          </div>
        ...
        {/* ================= DATA PRIVACY LEGAL TEXT VIEWPORT ================= */}
